...
const { readFile, parseJSON } = require("../src/helpers/helpers");
...
describe("Booking days module", () => {
    ...
    describe("check is date interval ok", () => {
        ...
        it("should fail if the year is 4 years in the future", done => {
            ...
            const dateName = "Date";
            const result =
                "Date year should not be set 3 years in the future\n";
            chai.expect(isDateIntervalOk(input, dateName)).to.equal(result);
            done();
        });
    });
            ...
            readFile("db/users.json")
                ...
                .then(users => {
                    validateUserExists(input, users).catch(err => {
                        ...
                        chai
                            .expect(err["status"])
                            .to.deep.equal(result["status"]);
                        chai
                            .expect(err["message"])
                            .to.deep.equal(result["message"]);
                        done();
                    });
                });
        });
